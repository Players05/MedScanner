...
function PresScan() {
  ...
  const handleLanguageConfirm = async () => {
    ...
    try {
      ...
      const result = await ApiService.analyzePrescription(file, selectedLanguage);
      ...
      localStorage.setItem('prescriptionResult', JSON.stringify(result.result));
      
      // Navigate to InfoBox after successful processing
      navigate('/infobox', { state: { result: result.result } });
    } catch (error) {
  ...
  const useCapturedImage = () => {
    if (file) {
      ...
      // The file is already set, so user can now click "Process Prescription"
    }
  };
    ...
    <div className="min-h-screen bg-teal-50 text-slate-900">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-16">
        ...
        <header className={`mx-auto mb-8 sm:mb-12 w-full max-w-5xl rounded-2xl bg-white/50 p-6 sm:p-8 text-left shadow-sm ring-1 ring-black/5 backdrop-blur transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
          <h1 className="text-center text-3xl sm:text-4xl lg:text-5xl font-semibold tracking-tight text-slate-900/90">Prescription Scanner</h1>
          <p className="mt-3 text-center text-base sm:text-lg lg:text-xl text-slate-600">Easily understand your prescription in a clear, simple way.</p>
        </header>

        {/* Upload section - two separate boxes for each feature */}
        <section aria-labelledby="upload-title" aria-describedby="upload-desc">
          <h2 id="upload-title" className="sr-only">Upload or scan prescription</h2>
          ...
          <div className={`mx-auto mt-4 sm:mt-6 max-w-3xl transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
            <div className="bg-white/90 rounded-2xl p-6 shadow-xl ring-1 ring-black/5 backdrop-blur">
              <div className="text-center mb-4">
                ...
                <p className="text-slate-600">Position your prescription in the frame</p>
              </div>
          ...
          <div className={`mx-auto mt-4 sm:mt-6 max-w-3xl transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
            <div className="bg-white/90 rounded-2xl p-6 shadow-xl ring-1 ring-black/5 backdrop-blur">
              <div className="text-center mb-4">
                ...
                <p className="text-slate-600">Review your captured prescription</p>
              </div>
            ...
            <div className="rounded-xl bg-white/80 p-4 shadow-sm ring-1 ring-black/5 backdrop-blur space-y-4">
              ...
              <div className="text-center">
                <button
                  ...
                  className="w-full sm:w-auto bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 disabled:from-gray-400 disabled:to-gray-500 text-white font-semibold py-3 px-6 sm:px-8 rounded-full transition-all duration-200 shadow-lg hover:shadow-xl disabled:cursor-not-allowed"
                >
                  {isProcessing ? (
        ...
        {file && (
          <section aria-labelledby="summary-title" className="mt-6 sm:mt-8">
            <h2 id="summary-title" className="text-lg sm:text-xl font-semibold mb-4">Prescription Summary</h2>
            <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
              <div className="rounded-lg border border-gray-200 bg-white p-3 sm:p-4 shadow-sm">
                <h3 className="font-medium text-gray-900 text-sm sm:text-base">Medicine Name & Purpose</h3>
                <p className="mt-2 text-xs sm:text-sm text-gray-700">Paracetamol — Used to reduce fever and relieve mild to moderate pain.</p>
              </div>
              <div className="rounded-lg border border-gray-200 bg-white p-3 sm:p-4 shadow-sm">
                <h3 className="font-medium text-gray-900 text-sm sm:text-base">Disease & Stage</h3>
                <p className="mt-2 text-xs sm:text-sm text-gray-700">Common cold — Early stage, symptomatic treatment recommended.</p>
              </div>
              <div className="rounded-lg border border-gray-200 bg-white p-3 sm:p-4 shadow-sm">
                <h3 className="font-medium text-gray-900 text-sm sm:text-base">Generic Alternatives</h3>
                <p className="mt-2 text-xs sm:text-sm text-gray-700">Acetaminophen 500mg tablets — Follow label directions.</p>
              </div>
              <div className="rounded-lg border border-gray-200 bg-white p-3 sm:p-4 shadow-sm sm:col-span-2">
                ...
                <p className="mt-2 text-xs sm:text-sm text-gray-700">Take as needed for fever or pain. Drink fluids and rest. If symptoms persist beyond 3 days or worsen, contact a healthcare provider.</p>
              </div>
            </div>
          </section>
        )}
